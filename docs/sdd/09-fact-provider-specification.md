...
- **`D-7`** — whether a missing or stale *required* fact may ever escalate to a human, and whether "evidence fact" is narrower than "required fact". §5.5 and `FactRequirement._check_escalation` (`registry/models.py:128-146`) currently make `required ∧ escalatable` unconstructible while `FactState.blocks` (`resolve/inputs.py:192`) only lets required facts reach the escalation arm, so the feature is inert. The reference fixture already contains the inert shape: the `rollback` class declares `escalate_on: ["SOLVER_TIMEOUT", "FACT_STALE"]` with `deploy_state` as `required: false, escalatable: true`, which `_check_escalatable_facts_exist` (`registry/models.py:277-292`) requires and which can never change a verdict. `D-7` is product's and security's, it is a hard precondition on `P1-18` alongside this document, and §3.3's `FactState` mapping is written to be correct under either answer.
