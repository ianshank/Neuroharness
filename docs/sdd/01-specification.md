...
| **Proposal digest** | SHA-256 over the RFC 8785 (JCS) canonical form of `proposal` + `context.actor` + `context.action_class` + `context.policy_bundle`. The identity of *what is being asked, by whom, under which rules*. Approvals bind to it. |
...
Inputs: infrastructure status, PDP outcomes, critic results with effective modes, fact status, registry entry (`approvable`, `escalate_on`, `mode`, `repair_budget`), `repair_iteration`. The procedure is monotone with respect to the safety order `ALLOW < REQUIRES_APPROVAL < REPAIR < ABSTAIN < DENY`.

0. **Mode normalization.** A hard critic whose *effective* mode (class mode, further restricted by per-critic mode) is `shadow` or `advisory` is treated as soft for resolution; its would-be effect is recorded. If the class mode is `halted` → `DENY` (`CLASS_HALTED`).
...
| `FR-04` | The harness MUST compute the **proposal digest** and the **envelope digest** (§2) over JCS-canonical JSON and stamp `action_id` (UUIDv7), `policy_bundle`, `registry`, critic versions (including solver versions) before evaluation. | Test (published test vectors) |
...
| ID | Requirement | Verify |
|---|---|---|
| `FR-80` | Each action class and each critic MUST have a rollout mode (`shadow`, `advisory`, `enforce`, `halted`); a per-critic mode MAY only be *less* enforcing than the class mode. `shadow` and `advisory` never block **on the verdict** but still require a persisted record and a shadow token; infrastructure failures block in every mode. In `shadow` no approval requests are created and no counterexamples are returned to the agent; `advisory` MAY surface typed warnings per `OQ-06`. Mode changes are signed registry changes. | Test, Fixtures `MUT-16`, `MUT-21` |
