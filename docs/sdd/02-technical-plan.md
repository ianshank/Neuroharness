...
A per-critic `mode` is independent of the class `mode`, and the loader does not compare them. Specification v0.4 made §5.3 step 0 demote a hard critic on the critic's **own** declared mode, so the `enforce` critics above would still be `enforce` if this class were moved to `shadow` — which is the point of a shadow rollout, and what `A-16`/`MUT-16` measure (`INV-11`). Nothing is given away: enforcement is capped at the broker, which reads the class mode, downstream of every critic. What the loader does reject is `halted` on a critic: `halted` is the class-level incident lever (`FR-49`), and a critic declaring it would be demoted to soft by step 0 and stop blocking altogether. The loader also rejects `escalate_on` on a non-approvable class (`FR-33`). A hard critic whose *own* mode is not `enforce` is treated as soft for resolution and records `would_be_verdict` (§5.3 step 0). Unregistered `(tool, intent)` → `ABSTAIN` in `strict` (`FR-31`). Classes with `effect_class` `none`/`read` take the policy-only fast path (`FR-35`).
