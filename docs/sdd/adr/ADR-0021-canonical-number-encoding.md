...
2. **The deviation is made unreachable rather than merely documented.** The two implementations diverge on exactly one class of value: one whose canonical form is an integer literal outside the IEEE-754 safe range (±(2^53 − 1)). The rule is therefore narrow: **a digested document may not contain a number that canonicalises to an integer literal outside the safe range**; values needing more range travel as strings. The digest functions reject such a value, naming its JSON Pointer path. `canonicalize` itself stays a faithful serialiser and does not impose the restriction, because the restriction is a property of what the harness agrees to *identify*, not of the encoding.

   **The test is on the canonical form, never on the Python type**, and that distinction is load-bearing rather than incidental. `1e20` is a `float` in Python, but RFC 8785 serialises it as `100000000000000000000`, and the broker parses those bytes back into an `int` before recomputing the digest (`FR-21`). A rule phrased over `isinstance(value, int)` would therefore admit the document at the gateway and refuse it at the broker — a fail-closed refusal of an envelope nobody tampered with, appearing only in production, on the side of the system that has already decided. The consequence is that the refused band is exactly the values that round-trip as integers: `float(2**60)` is refused because it renders as `1152921504606847000`, while `1e21` and `1e300` are permitted because ECMAScript renders them in exponential form and they round-trip as floats on both sides.
...
- **Phrase the restriction over the Python type (`isinstance(value, int)`).** Rejected, and it is the alternative most likely to be reached for: it reads as the narrower, more faithful rule. It is gateway/broker asymmetric, for the reason in decision point 2, and the asymmetry surfaces only after a decision has been made.
...
- Neutral, and worth stating because it surprises: the refused set is not "large numbers". `1e300` is digestible and `float(2**60)` is not, because the rule is about the rendered form and not about magnitude.
...
| Version | Date | Change |
|---|---|---|
| 0.2 | 2026-09-19 | Round-four review found decision point 2 describing a rule the implementation deliberately does not follow: it said floats "are permitted" and stated the restriction over integers, while `renders_as_unsafe_integer` has always tested the canonical form regardless of type. The code was right and this document was wrong; the reasoning it lacked (the broker re-parses canonical bytes, so a type-keyed rule is gateway/broker asymmetric) is now recorded here rather than only in the docstring. |
