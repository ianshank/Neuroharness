...
- **Corrected after review, and worth recording because the gap was in this decision's own claim.** The first implementation surveyed keyword *names* and not their *values*, so `type` passed the survey and `_JSON_TYPES.get()` then returned `None` inside the evaluation and skipped the type check for that subschema — silently. `{"replicas": {"type": "int"}}` — the typo for `integer` a schema author actually makes — accepted a string where the registry had asked for a number, with no violation reported. The list form `{"type": ["string", "null"]}` is legal JSON Schema this evaluator does not implement and fell through the same way. Both are now refused by the survey as `UNSUPPORTED_SCHEMA`, which is where "can the whole schema be applied?" is answered. The lesson generalises past this ADR: *a closed vocabulary is not closed until the values inside it are closed too*, and a check that degrades to "no opinion" on its own miss is the fail-open shape this decision exists to refuse.
